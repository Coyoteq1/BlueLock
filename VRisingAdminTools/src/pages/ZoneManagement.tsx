...
export default function ZoneManagement({ serverUrl, apiKey }: ZoneManagementProps) {
    ...
    <FlexColumn gap={24}>
      ...
      <Card>
        ...
        <Grid columns={3} gap={16}>
          <div>
            <label style={{ fontSize: '12px', color: '#8c91a0', display: 'block', marginBottom: '4px' }}>
              Glow Spacing (meters)
            </label>
            <ComponentBrowserInput
              type="number"
              value={glowSpacing}
              onChange={(e) => setGlowSpacing(parseFloat(e.target.value))}
              min={1}
              max={20}
            />
          </div>
          <div>
            ...
            <ComponentBrowserSelect
              value={glowPrefab}
              onChange={(e) => setGlowPrefab(e.target.value)}
            >
      ...
      <Card>
        ...
        {zones.length === 0 ? (
          <Text color="#6c7080">No zones configured.</Text>
        ) : (
