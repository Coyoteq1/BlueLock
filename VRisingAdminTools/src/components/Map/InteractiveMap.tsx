...
const DEFAULT_ZONES: MapZone[] = [
  { id: 'arena-main', x: 0, y: 0, width: 200, height: 200, type: 'arena', name: 'Main Arena' },
  { id: 'spawn-north', x: -300, y: -200, width: 100, height: 100, type: 'spawn', name: 'North Spawn' },
  { id: 'spawn-south', x: 200, y: 300, width: 100, height: 100, type: 'spawn', name: 'South Spawn' },
];

const DEFAULT_MARKERS: MapMarker[] = [
  { id: 'p1', x: 50, y: 50, type: 'player', name: 'Player 1' },
  { id: 'p2', x: -50, y: 100, type: 'player', name: 'Player 2' },
  { id: 'p3', x: 100, y: -50, type: 'player', name: 'Player 3' },
];
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', width, height }}>
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '4px', color: '#ffffff', fontWeight: 'bold' }}>Legend</div>
        {Object.entries(COLORS).map(([type, color]) => (
          <div key={type} style={{ display: 'flex', alignItems: 'center', gap: '6px', margin: '2px 0' }}>
            <span style={{ width: '10px', height: '10px', background: color, borderRadius: '2px' }} />
            <span style={{ color: '#8c91a0', textTransform: 'capitalize' }}>{type}</span>
          </div>
        ))}
      </div>
      ...
      {(hoveredMarker || hoveredZone) && (
        <div style={{
          ...
          <div style={{ color: COLORS[hoveredMarker?.type || hoveredZone?.type || ''], fontSize: '11px', marginTop: '2px' }}>
            {hoveredMarker?.type || hoveredZone?.type}
          </div>
          {hoveredMarker?.data && (
            <div style={{ color: '#8c91a0', fontSize: '10px', marginTop: '4px' }}>
