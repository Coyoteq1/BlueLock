...
namespace VAuto.Core.Services
{
    ...
    public static class EntitySpawner
    {
        ...
        private static void ApplyBuffToEntity(Entity entity, int buffId)
        {
            try
            {
                var buffGuid = new PrefabGUID(buffId.Value);
                
                // Check if we can use the buff system
                if (BuffSystem.Instance != null)
                {
                    BuffSystem.Instance.ApplyBuff(entity, buffGuid);
                    Plugin.Log.LogDebug($"[EntitySpawner] Applied buff {buffId} to entity");
                }
                else
                {
                    // Fallback: set buff component directly
                    var buff = new Buff
                    {
                        BuffGuid = buffGuid,
                        Duration = 30f,
                    ...
                    _entityManager.SetComponentData(entity, buff);
                    Plugin.Log.LogDebug($"[EntitySpawner] Set buff {buffId} component on entity");
                }
            }
            catch (Exception ex)
            {
