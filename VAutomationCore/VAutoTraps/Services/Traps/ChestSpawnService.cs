...
namespace VAuto.Core.Services
{
    ...
    public static class ChestSpawnService
    {
        ...
        public static void Initialize(CoreLogger log)
        {
            ...
            _initialized = true;
            _log = log;
            _log.LogInfo("[ChestSpawnService] Initialized - chest spawning ready");
        }
        ...
        public static EntityReference SpawnChest(float3 position, ulong spawnedBy, ChestRewardType type)
        {
            lock (_lock)
            {
                ...
                _log.LogInfo($"[ChestSpawnService] Spawned {type} chest at {position}");
                _log.LogInfo($"[ChestSpawnService]   Contents: {chest.Contents}");
            ...
            lock (_lock)
            {
                foreach (var kvp in _chests)
                {
                    ...
                    if (math.distance(playerPosition, chestPos) <= 2f)
                    {
                        ...
                        _log.LogInfo($"[ChestSpawnService] Player {playerId} looted chest at {chestPos}");
            ...
            lock (_lock)
            {
                foreach (var kvp in _chests)
                {
                    if (math.distance(position, kvp.Key) <= maxRadius)
                    {
                        ...
                        _log.LogInfo($"[ChestSpawnService] Removed chest at {kvp.Key}");
                        return true;
                    }
                }
                return false;
        ...
        public static void ClearAll()
        {
            ...
            {
                _chests.Clear();
                _log.LogInfo("[ChestSpawnService] All chests cleared");
            }
        }
