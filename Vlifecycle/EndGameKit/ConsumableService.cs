...
namespace VAuto.EndGameKit.Services
{
    ...
    {
        private readonly ProjectM.Scripting.ServerGameManager? _serverGameManager;
        ...
        public ConsumableService(ProjectM.Scripting.ServerGameManager? serverGameManager, EntityManager entityManager)
        {
        ...
        /// </summary>
        public void UpdateServerGameManager(ProjectM.Scripting.ServerGameManager? serverGameManager)
        {
            _serverGameManager = serverGameManager;
        }
        ...
        public bool Apply(Entity player, PrefabGUID itemGuid)
        {
            ...
            {
                Plugin.Log.LogWarning($"[ConsumableService] Invalid consumable GUID: {itemGuid.GuidHash}");
                return false;
            }
            ...
            try
            {
                ...
                if (!given)
                {
                    Plugin.Log.LogWarning($"[ConsumableService] Failed to give consumable {itemGuid.GuidHash} to player {player.Index}");
                    return false;
                }
                ...
                if (!used)
                {
                    Plugin.Log.LogWarning($"[ConsumableService] Failed to use consumable {itemGuid.GuidHash} on player {player.Index}");
                    return false;
                }

                Plugin.Log.LogDebug($"[ConsumableService] Applied consumable {itemGuid.GuidHash} to player {player.Index}");
                return true;
            }
            catch (Exception ex)
            {
                Plugin.Log.LogWarning($"[ConsumableService] Exception applying consumable {itemGuid.GuidHash}: {ex.Message}");
                return false;
            }
        }
