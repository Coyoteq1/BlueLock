...
namespace VAuto.EndGameKit.Services
{
    ...
    public class EquipmentService
    {
        ...
        public EquipmentService(ProjectM.Scripting.ServerGameManager? serverGameManager, EntityManager entityManager)
        {
        ...
        public int EquipKit(Entity player, Dictionary<EquipmentSlot, PrefabGUID> equipment)
        {
            ...
            foreach (var (slot, guid) in equipment)
            {
                ...
                {
                    Plugin.Log.LogWarning($"[EquipmentService] Failed to equip {guid.GuidHash} to slot {slot}: {ex.Message}");
                }
            }
        ...
        public bool EquipItem(Entity player, string slotName, PrefabGUID itemGuid)
        {
            ...
            {
                _serverGameManager.EquipItem(player, itemGuid, slot.ToSlotIndex(), true);
                Plugin.Log.LogDebug($"[EquipmentService] Equipped {itemGuid.GuidHash} to slot {slotName}");
                return true;
            }
            catch (Exception ex)
            {
                Plugin.Log.LogWarning($"[EquipmentService] Failed to equip {itemGuid.GuidHash} to slot {slotName}: {ex.Message}");
                return false;
            }
        }
