...
namespace VAuto.Zone.Commands
{
    ...
    public static class ArenaCommands
    {
        ...
        [Command("arena glow choices", shortHand: "agc", description: "List available glow prefab choices", adminOnly: false)]
        public static void ArenaGlowChoices(ChatCommandContext ctx)
        {
            var glowService = new GlowService();
            ctx.Reply("[Arena] Available glow prefabs:");
            foreach (var (name, prefab) in glowService.ListGlowChoices())
            {
        ...
        [Command("arena glow testspawn", shortHand: "agts", description: "Test spawn glow border with specific prefab", adminOnly: true)]
        public static void ArenaGlowTestSpawn(ChatCommandContext ctx, string prefabName)
        {
            var configPath = GetTerritoryConfigPath();
            
            // Use GlowService to resolve prefab name
            var glowService = new GlowService();
            var prefabGuid = glowService.GetGlowPrefab(prefabName);
            
            if (!prefabGuid.IsEmpty())
            {
                ctx.Reply($"[Arena] Found '{prefabName}' -> GUID {prefabGuid.GuidHash}");
            }
            else
            {
                // Try resolving directly from prefab collection using name hash
                var hashCode = prefabName.GetHashCode();
                if (VRCore.PrefabCollection._PrefabGuidToEntityMap.TryGetValue(new PrefabGUID(hashCode), out _))
                {
                    ctx.Reply($"[Arena] Found '{prefabName}' -> HashCode {hashCode}");
                }
                else
                {
                    ctx.Reply($"[Arena] ERROR: Prefab '{prefabName}' not found");
                    return;
                }
            }
            
            if (ArenaGlowBorderService.SpawnBorderGlows(configPath, prefabName, 3f, out var error))
            {
                ctx.Reply($"[Arena] Test spawn OK with '{prefabName}'");
            }
            else
            {
                ctx.Reply($"[Arena] ERROR: {error}");
        ...
        [Command("dump prefabs", shortHand: "dp", description: "Dump all prefab GUIDs to file", adminOnly: true)]
        public static void DumpPrefabs(ChatCommandContext ctx)
        {
            var sb = new StringBuilder();
            foreach (var kvp in VRCore.PrefabCollection._PrefabGuidToEntityMap)
            {
                sb.AppendLine($"{kvp.Key.GuidHash}");
            }
            File.WriteAllText("C:/PrefabsDump.txt", sb.ToString());
            ctx.Reply($"[Arena] Dumped {VRCore.PrefabCollection._PrefabGuidToEntityMap.Count} prefabs to C:/PrefabsDump.txt");
        }
