...
namespace VAuto.Commands.Core
{
    ...
    public static class ZoneCommands
    {
        ...
        [Command("ea", description: "Shortcut for .enterarena", adminOnly: false)]
        public static void ShortEnterArena(ChatCommandContext ctx)
        {
            EnterArena(ctx);
        }
        ...
        private static float3? GetPlayerPosition(ChatCommandContext ctx)
        {
            try
            {
                ...
                {
                    Plugin.Log.LogWarning("[Zone] Sender character entity not found");
                    return null;
                }
                ...
                {
                    return entityManager.GetComponentData<Translation>(characterEntity).Value;
                }
