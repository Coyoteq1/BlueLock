...
namespace VAuto.EndGameKit.Services
{
    ...
    public class JewelService
    {
        ...
        public bool AttachJewel(Entity player, PrefabGUID jewelGuid)
        {
            ...
            {
                Plugin.Log.LogWarning($"[JewelService] Invalid jewel GUID: {jewelGuid.Value}");
                return false;
            }
            ...
            for (int i = 0; i < sockets.Length; i++)
            {
                if (sockets[i].Jewel.Equals(PrefabGUID.Empty))
                {
                    ...
                    Plugin.Log.LogDebug($"[JewelService] Attached jewel {jewelGuid.Value} to socket {i} for player {player.Index}");
                    return true;
                }
            }

            Plugin.Log.LogWarning($"[JewelService] No available sockets for player {player.Index}");
            return false;
        }
